...
## Представление истории для суммаризации

- Форматтер меняет только вход суммаризатора, не исходные сообщения и transcript. Для результата инструмента сохраняется лимит 500 символов. В выводе `cli_exec` отбираются диагностические строки, явные exit code и итоги тестов; ошибки имеют приоритет перед предупреждениями. Выдержки остаются в исходном порядке, сохраняют переносы и отступы, а пропуски отмечаются явно. Совпадение с диагностическим шаблоном не считается доказательством статуса операции.
- Для неизвестных форматов, содержимого файлов и слишком длинных диагностических строк остаётся нормализованный head+tail (30% начала / 70% конца доступного места). Это сжатие с потерями: вложенный JSON и все ошибки не гарантированно помещаются в бюджет.
- Вызовы и результаты связываются через `tool_call_id` локальными метками `c1`, `c2`, … внутри одного запроса суммаризатора. Несопоставленный результат отмечается `unmatched`. Список вызовов ограничен 320 символами: бюджет распределяется между вызовами, команда/путь/запрос и параметры идут раньше больших текстов правок. Сокращения аргументов и пропущенные вызовы обозначаются явно. Известные секретные поля и строковые шаблоны маскируются до сокращения; это не универсальное распознавание секретов в произвольном тексте.
- Событие `summary_compacted` содержит локальные оценки токенов предыдущей памяти, истории, snapshot и полного промпта, а также число и длительность вызова суммаризации. `summary_memory_folded` отдельно фиксирует вызов fold, его сбой, длительность и жёсткую обрезку. Эти метрики не содержат тексты запросов и не являются тарифицируемым usage провайдера; внутренние retry провайдера не подсчитываются.
