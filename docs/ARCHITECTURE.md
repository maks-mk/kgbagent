...
Для предотвращения переполнения контекста при получении огромных ответов от инструментов (логи сборки, поисковая выдача, листинги директорий) используется двухэтапный pipeline сжатия:

1. **Семантическое сжатие (Headroom)**:
   - Если `ENABLE_HEADROOM_COMPRESSION=true`, вывод передаётся в `ContentRouter`.
   - Роутер определяет тип контента (текст, JSON, таблица, лог) и применяет соответствующий компрессор.
   - Для логов дополнительно пробуется `LogCompressor` с дедупликацией повторяющихся предупреждений.
   - Результат принимается, если он сохраняет критическую диагностику (ошибки, traceback) и исходный статус `ERROR[TYPE]`.
   - Явные lossless-фолды принимаются даже при очень малом размере.

2. **Детерминированное сокращение (Fallback)**:
   - Если семантическое сжатие недоступно, отклонено или результат всё ещё слишком велик, применяется жесткое сокращение.
   - Для `cli_exec` сохраняются head, tail и все найденные диагностические строки.
   - Для MCP и прочих инструментов используется `middle_fit` (сохранение начала и конца с маркером `[OMITTED N chars]`).
   - Итоговый результат всегда ограничен лимитом `MAX_TOOL_OUTPUT`.

Файловые операции (чтение/запись/правка) исключены из семантического сжатия для обеспечения точности последующих правок.
