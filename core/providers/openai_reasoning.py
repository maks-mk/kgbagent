...
def create_openai_chat_model(config: AgentConfig, *, api_key_override: str | None = None) -> BaseChatModel:
    ...
    provider_model_supports_reasoning = isinstance(provider_config, dict)
    provider_uses_toggle = isinstance(provider_config, dict) and provider_config.get("mode") == "toggle"
    if provider_model_supports_reasoning and (reasoning_enabled or provider_uses_toggle):
        debug_event(
            "reasoning_request",
        ...
        reasoning_logger.debug(
            "openai reasoning registry match model=%s base_url=%s provider_id=%s mode=%s param=%s effort=%s enabled=%s",
            config.openai_model,
            ...
            provider_config.get("param") if isinstance(provider_config, dict) else None,
        ...
        reasoning_logger.debug(
            "openai reasoning skipped model=%s base_url=%s provider_id=%s reasoning_enabled=%s model_supported=%s",
            config.openai_model,
            config.openai_base_url,
            provider_config.get("id") if isinstance(provider_config, dict) else None,
            reasoning_enabled,
